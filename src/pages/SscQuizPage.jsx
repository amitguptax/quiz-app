...
const SscQuizPage = () => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        const filePath = `../data/ssc/${exam}/${subject}.js`;
        const module = await import(/* @vite-ignore */ filePath);
        setQuestions(module.default[subtopic]);
      } catch (err) {
        console.error("❌ Failed to load SSC quiz file:", err);
        setQuestions([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "800px", margin: "auto" }}>
      <Helmet>
        <title>{`SSC ${exam.toUpperCase()} Quiz - ${subtopic}`}</title>
      </Helmet>
      ...
      {!quizFinished && (
        <>
          <h2>📘 SSC {exam.toUpperCase()} - {subject}</h2>
