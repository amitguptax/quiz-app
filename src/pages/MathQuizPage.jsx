...
import mathtopicMap from "../data/mathtopicMap";
import topicDataMap from "../data/math/topicDataMap";
...
const folderName = mathtopicMap[classId]?.[topic];  // ✅ Correct
...
useEffect(() => {
  console.log("📛 Subtopic slug:", subtopic);
  const data = topicDataMap[subtopic];
  console.log("📦 Loaded topic data:", data);

  if (data && data.questions) {
    setQuestions(data.questions);
  } else {
    console.warn("⚠️ No data found for subtopic:", subtopic);
    setQuestions([]);
  }
  setLoading(false);
}, [subtopic]);
  ...
  if (loading) {
    return <p style={{ padding: "20px" }}>⏳ Loading questions...</p>;
  }
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "800px", margin: "auto" }}>
      <Helmet>
        <title>Math Quiz - Class {classId} - {subtopic}</title>
      </Helmet>
  ...
  <p style={{ fontSize: "18px", fontWeight: "bold", color: "#1e90ff" }}> ⏱️ {labels[language].timeTaken}: {formatTime(timeTaken)}</p>
  ...
  {question.options.map((opt, idx) => {
    ...
    return (
      ...
      {/* Show results after quiz finishes */}
      {quizFinished && (
  <div style={{ marginTop: "20px" }}>
    ...
    <div style={{ marginTop: "20px", background: "#f0f8ff", padding: "15px", borderRadius: "10px" }}>
      <h3>🏆 Leaderboard</h3>
